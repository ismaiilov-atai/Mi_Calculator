...
class DeprecateParser {
	...
	private enum class Operators(val sign: Char) {
		PLUS('+'), MINUS('-'), MULTIPLY('*'), DIVISION('/'), POWER('^'), EXPONENTIAL('E');
	}
		...
		if (operator == Operators.PLUS) {
			...
			if (position == 0) {
				...
				for (legalOp in Operators.values()) {
					if (prevOperator == legalOp.sign) return false
				}
			...
			while (position > 0) {
				if (isOperator(operator, expression, position)) {
					...
					val res = when (operator) {
						...
						Operators.DIVISION -> {
							if (value1 == 0.0) throw ArithmeticException("Divide By Zero")
							value0 / value1
						}
